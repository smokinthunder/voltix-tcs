window.VoltixComponents = (function () {
  ...
  function renderNavbar() {
    ...
    const user = window.VoltixCommon.getCurrentUser(); const role = user?.role || 'Customer'; const avatar = window.VoltixCommon.initials(user?.name || role);
    root.innerHTML = `<nav class="navbar navbar-expand-lg dashboard-navbar sticky-top"><div class="container-xxl"><a class="navbar-brand d-flex align-items-center gap-2" href="${role === 'Admin' ? 'admin-dashboard.html' : 'dashboard.html'}"><div class="brand-mark"><i class="bi bi-lightning-charge-fill"></i></div><div><span class="brand-text">Voltix</span><small class="brand-subtext d-block">Electricity Billing</small></div></a><button class="navbar-toggler border-0 shadow-none" type="button" data-bs-toggle="collapse" data-bs-target="#mainNav"><span class="navbar-toggler-icon custom-toggler-icon"></span></button><div class="collapse navbar-collapse" id="mainNav"><ul class="navbar-nav mx-auto mb-3 mb-lg-0 nav-pill-group">${navItems(role).map(item => `<li class="nav-item"><a class="nav-link ${p === item[0] ? 'active' : ''}" href="${item[2]}">${item[1]}</a></li>`).join('')}</ul><div class="d-flex align-items-center gap-3"><button class="icon-btn" type="button"><i class="bi bi-bell"></i></button><div class="profile-chip"><div class="profile-avatar">${avatar}</div><div class="profile-info"><span class="profile-name">${window.VoltixCommon.escapeHtml(user?.name || role)}</span><small class="profile-role">${role}</small></div></div><button class="btn btn-outline-soft btn-sm-custom" id="logoutBtn" type="button"><i class="bi bi-box-arrow-right me-2"></i>Logout</button></div></div></div></nav>`;
  }
  function renderFooter() { const root = document.getElementById('footer-root'); if (root) root.innerHTML = '<footer class="site-footer"><div class="container-xxl"><p class="mb-0 text-center">Built as a static multi-page demo with reusable JavaScript components and localStorage-backed sample data.</p></div></footer>'; }
  function attachEvents() { document.addEventListener('click', e => { const btn = e.target.closest('#logoutBtn'); if (btn) window.VoltixCommon.logout(); }); }
